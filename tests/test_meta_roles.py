...
from kaidzen.roles.meta import load_meta_prompt
...
from kaidzen.roles.meta.mutator import (MutationProposal, run_mutator, to_patch)
...
def test_meta_judge_signature_has_no_room_for_context():
    ...
    assert params == ["backend", "candidate", "report_a", "report_b"]
...
def test_meta_judge_can_return_tie(candidate):
    ...
    out = run_meta_judge(llm, candidate, report_a="a", report_b="b")
...
def test_mutator_attempt_changes_the_prompt(candidate):
    ...
    run_mutator(llm, candidate, diagnosis=_diagnosis(), attempt=0)
    run_mutator(llm, candidate, diagnosis=_diagnosis(), attempt=1)
...
def test_deep_roles_declare_their_effort(candidate):
    ...
    run_diagnostician(llm, candidate, metrics=RunMetrics(), reports=["о"])
    run_mutator(llm, candidate, diagnosis=_diagnosis(), attempt=0)
